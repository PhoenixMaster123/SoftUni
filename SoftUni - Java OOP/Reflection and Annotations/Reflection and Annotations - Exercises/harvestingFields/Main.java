...
public class Main {
	public static void main(String[] args) {
		...
		while (!"HARVEST".equals(input)) {
			...
			if("all".equals(input)) {
				Arrays.stream(declaredFields).forEach(field -> System.out.printf("%s %s %s\n",
						Modifier.toString(field.getModifiers()), field.getType().getSimpleName(), field.getName()));
			} else {
				String accessModifier = input;
				Arrays.stream(declaredFields).filter(field -> Modifier.toString(field.getModifiers()).equals(accessModifier))

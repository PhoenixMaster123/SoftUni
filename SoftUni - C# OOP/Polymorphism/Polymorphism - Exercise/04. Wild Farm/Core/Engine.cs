    ...
    public class Engine : IEngine
    {
        ...
        public void Start()
        {
            ...
            while ((command = Console.ReadLine()) != "End")
            {
                try
                {
                    string[] animalsValue = command.Split();
                    string[] foodValue = Console.ReadLine().Split();

                    Animal animal = BuildAnimalUsingFactory(animalsValue);
                    Food food = this.foodFacctory.CreateFood(foodValue[0], int.Parse(foodValue[1]));
